...
@interface WorkDetailVC ()<UIWebViewDelegate,CLLocationManagerDelegate, UIImagePickerControllerDelegate,CLLocationManagerDelegate,UIAlertViewDelegate>{
...
- (void)setNav{
    ...
    if ([self.title isEqualToString:@"新文件列表"]) {
        
        rightButton = [[UIBarButtonItem alloc]initWithTitle:@"已读文件" style:UIBarButtonItemStyleDone target:self action:@selector(rightClick1)];
...
- (void)rightClick1{
    ...
    WorkNavPushVC *nav = [[WorkNavPushVC alloc] init];
    nav.urlStr = @"oa/readTodoListHad.html";
...
{//多次调用shouldStartLoad,是因为js文件里使用了自定义url的方式
   ...
   // requestString = [requestString stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    NSArray *unidArray = [transStr componentsSeparatedByString:@"?unid="];
    NSString *urlStr;
    BOOL isPush = false;
    ...
    if (isPush) {
        ...
        NSString *tempStr =  [NSString stringWithFormat:@"%@?unid=%@",urlStr,unidArray.lastObject];
