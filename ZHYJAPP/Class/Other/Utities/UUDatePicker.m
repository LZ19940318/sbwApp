...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.minLimitDate) {
        ...
        self.minLimitDate = [self dateFromString:@"194001010000" withFormat:@"yyyyMMddHHmm"];
        minDateModel = [[UUDatePicker_DateModel alloc]initWithDate:self.minLimitDate];
    }
    ...
    if (!myPickerView) {
        myPickerView = [[UIPickerView alloc]initWithFrame:self.frame];
...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x, 20, 20, 20)];
    label.text = name;
