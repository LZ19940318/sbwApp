...
@interface EaseMessageViewController ()<EaseMessageCellDelegate, UIDocumentPickerDelegate>
{
                           ...
                           conversationType:(EMConversationType)conversationType
{
    ...
    if (self) {
        tempUnid = conversationChatter;
        self.unid = conversationChatter;
...
- (void)getTitle{
    ...
    for (int i = 0; i < mArray.count; i++) {
        ...
        if ([[dic[@"ImUserName"] lowercaseString] isEqualToString:tempUnid]) {
            self.title = dic[@"cnName"];
        }
...
//    tapRecognizer
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(tap:) name:@"tapRecognizer" object:nil];
...
- (void)tap:(NSNotification *)not{
    
    UITapGestureRecognizer *recognizer = not.object;
...
//    }
    ...
    switch (model.bodyType) {
        ...
        {//遵循代理
            ...
            if (model.message.ext != NULL) {
                
                missionTaskVC *missVC = [[missionTaskVC alloc] init];
                NSString *str1 = [model.message.ext[@"attr1"] stringByReplacingOccurrencesOfString:@"#"withString:@"/"];
                NSString *str2 = [model.message.ext[@"attr2"] stringByReplacingOccurrencesOfString:@"#"withString:@"/"];
                NSString *str3 = [model.message.ext[@"attr3"] stringByReplacingOccurrencesOfString:@"#"withString:@"/"];
//                str3 = [str3 stringByReplacingOccurrencesOfString:@"&appdb="withString:@""];
                // 审批  邮件
                if (str1) {
...
//                    [nav pushViewController:vc animated:YES];
                    Work_Document_VC *vc = [[Work_Document_VC alloc]init];
                    vc.urlStr = str1;
                    [nav pushViewController:vc animated:YES];
                    tabBarVC.selectedIndex = 2;
                ...
                if(str3){
                    ...
                    // ZHYJWebViewController   ReportDetaiVC
                    if ([str3 hasPrefix:@"mail/mailList.html"]) {
                       
//                        ZHYJWebViewController *vc = [[ZHYJWebViewController alloc]init];
//                      
...
//                        NSArray *array = self.navigationController.viewControllers;
                        ...
                        // 根据当前 的页面，判断是否显示 返回按钮
//                        if(tabBarVC.selectedIndex == 4 && [array[0] isKindOfClass:[MyVC class]]){
//                            
...
//                        }
                    ...
                    if ([str3 hasPrefix:@"mail/mail.html"]) {
                        ReportDetaiVC *vc = [[ReportDetaiVC alloc]init];
                        vc.urlS = str3;
                        [self.navigationController pushViewController:vc animated:YES];
...
//                    missVC.urlStr = str3;
                    //            [self navigationController:missVC animated:YES completion:nil];
                ...
                if([array count]>0  && model.message.ext == NULL)
                {
                    //自定义在app内打开
                    NSLog(@"message111===%@", str);
                    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://www.jianshu.com/p/7909656c96f5"]];
                }
...
//            if (messageModel.message.ext != NULL) {
//               
//            }
...
- (void)moreViewFileTransferAction:(EaseChatBarMoreView *)moreView{
    ...
    [self.chatToolbar endEditing:YES];
    NSArray *documentType = @[@"public.content", @"public.text", @"public.source-code", @"publick.image", @"public.audiovisual-content", @"com.adobe.pdf", @"com.apple.keynote.key", @"com.microsoft.word.doc", @"com.mircrosoft.excel.xls", @"com.mircrosoft.powerpoint.ppt"];
    UIDocumentPickerViewController *documentPicker = [[UIDocumentPickerViewController alloc] initWithDocumentTypes:documentType inMode:UIDocumentPickerModeOpen];
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentAtURL:(NSURL *)url{
    ...
    if (fileUrlAuthozied) {
        ...
        [fileCoordinator coordinateReadingItemAtURL:url options:0 error:&error byAccessor:^(NSURL *newURL){
            ...
            EMFileMessageBody *chatFile = [[EMFileMessageBody alloc] initWithData:data displayName:[[url path] lastPathComponent]];
...
- (void)didReceiveMessages:(NSArray *)aMessages
{
    for (EMMessage *message in aMessages) {
        NSLog(@"mesage-ext3===%@", message.ext[@"attr1"]);
