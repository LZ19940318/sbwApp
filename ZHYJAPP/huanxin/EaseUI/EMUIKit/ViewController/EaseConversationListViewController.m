...
//
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector(conversationListViewController:latestMessageTimeForConversationModel:)]) {
        ...
        cell.avatarView.image = model.avatarImage;
    } else {
//        cell.timeLabel.text = [self _latestMessageTimeForConversationModel:model];
        cell.avatarView.image = model.avatarImage;
...
- (void)tableViewDidTriggerHeaderRefresh
{
    ...
    for(int i = 0; i< tempArray.count;i++){
        EaseConversationModel *tempModel = tempArray[i];
        NSInteger num = 0;
        for (int j = 0; j < tempArray.count; j++) {
            
            EaseConversationModel *tempModel1 = tempArray[j];
            if ([tempModel.conversation.conversationId isEqualToString:tempModel1.conversation.conversationId]) {
                
                num++;
                if (num == 2) {
                    [self.dataArray removeObject:tempArray[j]];
                }
        ...
        //tempModel.conversation.conversationId
...
//    NSArray *conversations1 = [[EMClient sharedClient].chatManager getAllConversations];
//    NSMutableArray *needRemoveConversations;
//    for (EMConversation *conversation in conversations1) {
...
- (void)remove{
    ...
    NSArray *conversations = [[EMClient sharedClient].chatManager getAllConversations];
    NSMutableArray *needRemoveConversations = [[NSMutableArray alloc]init];
    NSMutableArray *listAry = [[NSMutableArray alloc]init];
    for (EMConversation *conversation in conversations) {
        
   
        [listAry addObject:conversation.conversationId];
    ...
    for (int i = 0; i < listAry.count; i ++) {
        ...
        NSString *tempStr = listAry[i];
        NSInteger num = 0;
        EMConversation *temp = [[EMConversation alloc]init];
        for (EMConversation *conversation in conversations) {
            temp = conversation;
            if ([conversation.conversationId isEqualToString:tempStr]) {
                num ++;
                temp = conversation;
        ...
        if (num == 2) {
            [needRemoveConversations addObject:temp];
        }
