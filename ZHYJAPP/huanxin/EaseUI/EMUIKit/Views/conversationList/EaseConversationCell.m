...
- (void)setModel:(id<IConversationModel>)model
{
    ...
    NSString *name = nil;
    NSString *nick = nil;
    if ([DataManager sharedDataManager].IMUserlist != nil) {
        for (NSDictionary *dict in [DataManager sharedDataManager].IMUserlist) {
            if ([dict[@"ImUserName"] isEqualToString:[DataManager sharedDataManager].userMsg[@"ImUserName"]]) {
                //匹配自己的IM-ID到聊天界面
                NSString *photoImage = [dict[@"photo"] stringByReplacingOccurrencesOfString:@"\r\n" withString:@""];
                [[DataManager sharedDataManager] saveUserImage:photoImage];
            }
            if ([[dict[@"ImUserName"] lowercaseString] isEqualToString:model.title]) {
                name = dict[@"cnName"];
                nick = dict[@"cnName"];
                model.avatarURLPath = [dict[@"photo"] stringByReplacingOccurrencesOfString:@"\r\n" withString:@""];
                if (![dict[@"photo"] isEqualToString:@""]) {
                    dataBaseStr = [[NSData alloc]
                                   initWithBase64EncodedString:[dict[@"photo"] stringByReplacingOccurrencesOfString:@"\r\n" withString:@""] options:NSDataBase64DecodingIgnoreUnknownCharacters];
                                    model.avatarImage = [UIImage imageWithData:dataBaseStr];
...
//                                model.avatarImage = [UIImage imageNamed:@"User"];
    ...
    if (self.showAvatar) {
        if ([dataBaseStr length] > 0){
            NSLog(@"////%lu", (unsigned long)dataBaseStr.length);
            ...
            self.avatarView.image = [UIImage imageWithData:dataBaseStr];
