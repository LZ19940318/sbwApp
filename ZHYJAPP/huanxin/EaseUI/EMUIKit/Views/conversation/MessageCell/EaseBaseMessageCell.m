...
//    _bubbleView.backgroundImageView.image =[UIImage imageNamed:@"workchoose"];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if (self.model.isSender) {
        ...
        for (NSDictionary *dict in [DataManager sharedDataManager].IMUserlist) {
            if ([[dict[@"ImUserName"] lowercaseString] isEqualToString:model.nickname]) {
                nameStr = dict[@"cnName"];

            }
        }
        if ([DataManager sharedDataManager].IMImagePath != NULL && ![[DataManager sharedDataManager].IMImagePath isEqualToString:@""]) {
            NSData *dataFromBase64Str = [[NSData alloc] initWithBase64EncodedString:[DataManager sharedDataManager].IMImagePath options:NSDataBase64DecodingIgnoreUnknownCharacters];
            self.avatarView.image = [UIImage imageWithData:dataFromBase64Str];
