...
@property (strong, nonatomic) NSMutableArray *listArray; // 数据源，从本地服务器取，存本地
...
//        _contactsSource = [NSMutableArray arrayWithArray:[DataManager sharedDataManager].IMUserlist];
//        NSLog(@"%@", _contactsSource[0]);
//        NSLog(@"%lu", (unsigned long)[DataManager sharedDataManager].IMUserlist.count);
...
- (NSMutableArray *)listArray{
    if (!_listArray) {
        _listArray = [[DataManager sharedDataManager] IMUserlistSort];
    }
    return _listArray;
}
...
- (void)viewDidLoad
{
    ...
    UIButton *backButton = [[UIButton alloc] initWithFrame:CGRectMake(-25, (44-20)/2, 15, 20)];
...
//    [self searchController];
    ...
    if ([_blockSelectedUsernames count] > 0) {
        for (NSString *username in _blockSelectedUsernames) {
            ...
            if (tmpArray && [tmpArray count] > 0 && self.selectedContacts.count != 0) {
...
//    if (section == 0) {
//        return [DataManager sharedDataManager].IMUserlist.count;
//    }
//    return 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"cell--%@", [DataManager sharedDataManager].IMUserlist[indexPath.row]);
    NSString *username = [[self.listArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row][@"cnName"];
    
    NSString *imagePath = [[self.listArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row][@"photo"];

    if (![imagePath isEqualToString:@""]) {
       NSData *dataFromBase64Str = [[NSData alloc]
                             initWithBase64EncodedString:[imagePath stringByReplacingOccurrencesOfString:@"\r\n" withString:@""] options:NSDataBase64DecodingIgnoreUnknownCharacters];
        cell.imageView.image = [UIImage imageWithData:dataFromBase64Str];
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_blockSelectedUsernames count] > 0) {
        NSString *username = [DataManager sharedDataManager].IMUserlist[indexPath.row][@"ImUserName"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   // id object = [[_dataSource objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
    id object = [[self.listArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
    if (self.mulChoice) {
...
- (void)reloadFooterView
{
    if (self.mulChoice) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            NSDictionary *dict = self.selectedContacts[i];
            //自己处理任务昵称和头像
            NSString  *nameS = dict[@"cnName"];
            NSString  *imagePath = dict[@"photo"];
            
                if (![imagePath isEqualToString:@""]) {
                    NSData *dataFromBase64Str = [[NSData alloc]
                                                 initWithBase64EncodedString:[imagePath stringByReplacingOccurrencesOfString:@"\r\n" withString:@""] options:NSDataBase64DecodingIgnoreUnknownCharacters];
                    remarkView.image = [UIImage imageWithData:dataFromBase64Str];
                    
                }else{
                    remarkView.image = [UIImage imageNamed:@"chatListCellHead.png"];
                }
            remarkView.remark = nameS;
            [self.footerScrollView addSubview:remarkView];
//            CGFloat w = self.footerScrollView.contentSize.width;
//            [self.footerScrollView scrollRectToVisible:CGRectMake(w - self.footerView.frame.size.width - 200 , 0, self.footerView.frame.size.width, self.footerView.frame.size.height) animated:NO];
//            [self.footerScrollView scrollRectToVisible:CGRectMake( - 200 , 0, self.footerView.frame.size.width, self.footerView.frame.size.height) animated:NO];
