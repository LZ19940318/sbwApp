        ...
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                for (NSDictionary *dict in [DataManager sharedDataManager].IMUserlist) {
                    if ([[dict[@"ImUserName"] lowercaseString] isEqualToString:username]) {
                        name = dict[@"cnName"];
                        nick = dict[@"cnName"];
                        
                        contactView.remark = nick;
                        if (![dict[@"photo"] isEqualToString:@""]) {
                            ...
                            NSLog(@"%@", contactView.image);
                            contactView.image = [UIImage imageWithData:dataFromBase64Str];
                        }else{
...
- (void)dissolveAction
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
        EMError *error = nil;
//        NSArray *array1 = [[EMClient sharedClient].chatManager getAllConversations];
//        NSLog(@"array1---%@", array1);
...
//        NSArray *array2 = [[EMClient sharedClient].chatManager getAllConversations];
//        NSLog(@"array1---%@", array2);
    });
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    {
        //delete
        if (_selectedContact.deleteContact != 0) {
