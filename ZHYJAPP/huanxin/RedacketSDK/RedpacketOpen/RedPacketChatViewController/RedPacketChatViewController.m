...
EaseMessageViewControllerDataSource, RedpacketViewControlDelegate, EaseMessageViewControllerDelegate,EaseMessageViewControllerDataSource>
/**
 *  发红包的控制器
 */
...
- (BOOL)messageViewController:(EaseMessageViewController *)viewController didSelectMessageModel:(id<IMessageModel>)messageModel{
    ...
    if (messageModel.message.ext != NULL) {
        ...
        NSString *str1 = [messageModel.message.ext[@"attr1"] stringByReplacingOccurrencesOfString:@"#"withString:@"/"];
        NSString *str2 = [messageModel.message.ext[@"attr2"] stringByReplacingOccurrencesOfString:@"#"withString:@"/"];
        NSString *str3 = [messageModel.message.ext[@"attr3"] stringByReplacingOccurrencesOfString:@"#"withString:@"/"];
        str3 = [str3 stringByReplacingOccurrencesOfString:@"&appdb="withString:@""];
        if (str1) {
            
            missVC.urlStr = str1;
//            [self presentViewController:missVC animated:YES completion:nil];
            [self.navigationController pushViewController:missVC animated:YES];

            
//            NSIndexPath *indexPaht = [self.tableView indexPathForSelectedRow ];
//            [self messageViewController:self canLongPressRowAtIndexPath:indexPaht];
        ...
        if([array count]>0  && messageModel.message.ext == NULL)
        {
            //自定义在app内打开
