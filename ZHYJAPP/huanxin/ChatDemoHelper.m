...
@interface ChatDemoHelper()<EMCallManagerDelegate, UIAlertViewDelegate>
{
    NSTimer *_callTimer;
}
...
- (void)didLoginFromOtherDevice
{
    ...
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"温馨提示") message:NSLocalizedString(@"loginAtOtherDevice", @"账号在其他地方登陆") delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    // 跳到登录界面
    ZHYJLoginViewController *loginVC = [[ZHYJLoginViewController alloc]init];
...
//    }
    ...
    if ([[UIApplication sharedApplication] applicationState] != UIApplicationStateActive) {
        _mainVC.callSession = aSession;
        UILocalNotification *notification = [[UILocalNotification alloc] init];
        notification.alertBody = @"你有一个新来电";
        notification.alertAction = @"打开";
        notification.timeZone = [NSTimeZone defaultTimeZone];
        notification.soundName = UILocalNotificationDefaultSoundName;
        NSDictionary *infodic = [NSDictionary dictionaryWithObjectsAndKeys:@"call", @"withCall", nil];
